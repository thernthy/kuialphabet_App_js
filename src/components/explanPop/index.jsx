...
export const ExplanationPopup = ({data, setIsExplanetion}) => {
    ...
    return(
        <div className="fixed inset-0 w-full bg-white flex items-center justify-center z-50 ">
            ...
            <div className="text-gray-700 w-full">
                <div className="md:grid md:grid-cols-2 md:gap-4 mb-4  w-full">
                    <div className="px-1 py-2 flex flex-col items-center justify-center gap-3">
                        <div className="max-h-80 min-h-40 max-w-96 min-w-32">
                            <img src={`${process.env.REACT_APP_BASED_URL}storage/${data[0]?.cover_photo}`} className=" w-56" alt="" />
                        </div>
                        <div className="px-2 py-1 flex flex-row items-center justify-start gap-3">
                            <AudioPlayer audioFile={`${process.env.REACT_APP_BASED_URL}storage/${data[0].audio_files}`} />
                            <a href={data[0]?.video_url} target="_blank" 
                            className=' no-underline text-white px-3 text-nowrap py-1.5 bg-red-500 rounded-md' rel="noopener noreferrer">
                        ...
                        <table className="min-w-full bg-white border border-gray-300">
                            <thead className="">
                                <tr className="w-full bg-gray-100 border-b border-gray-300">
                                    <th className="text-center border-r border-gray-300 capitalize">{t("homePage.kui_ator")}</th>
                                    <th className="text-center border-r border-gray-300 capitalize">{t("homePage.kui_atreur")}</th>
